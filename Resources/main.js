...
const screenshotButtonText = screenshotButton?.textContent || "📷";
const regionScreenshotButtonText = regionScreenshotButton?.textContent || "✂️";
let screenshotBusy = false;

function resetScreenshotButtons() {
  screenshotBusy = false;
  if (screenshotButton) {
    screenshotButton.disabled = false;
    screenshotButton.classList.remove("uploading");
    screenshotButton.textContent = screenshotButtonText;
  }
  if (regionScreenshotButton) {
    regionScreenshotButton.disabled = false;
    regionScreenshotButton.classList.remove("uploading");
    regionScreenshotButton.textContent = regionScreenshotButtonText;
  }
}
...
ipcRenderer.on(channel, (_event, json) => {
  try {
    ...
    switch (payload.type) {
      case "screenshot-uploading":
        if (payload.mode === "fullscreen" && screenshotButton) {
          screenshotButton.classList.add("uploading");
          screenshotButton.textContent = "上传中";
        }
        if (payload.mode === "region" && regionScreenshotButton) {
          regionScreenshotButton.classList.add("uploading");
          regionScreenshotButton.textContent = "上传中";
        }
        break;

      case "screenshot-failed":
      case "screenshot-complete":
        resetScreenshotButtons();
        break;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  screenshotButton.addEventListener("click", () => {
    ...
    screenshotBusy = true;
    screenshotButton.classList.add("uploading");
    screenshotButton.textContent = "截图中";
  ...
  regionScreenshotButton?.addEventListener("click", () => {
    ...
    screenshotBusy = true;
    regionScreenshotButton.classList.add("uploading");
    regionScreenshotButton.textContent = "选区中";
